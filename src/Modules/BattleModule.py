...
class BattleModule:
    ...
    @staticmethod
    def _dice(side_counter: int, throw_counter: int) -> int:
        """
    ...
    def _hit_result(self, whose_turn: str) -> dict:
        """
        ...
        def player_attack() -> dict:
            ...
            if attack["attack_type"] == "Melee":
                ...
                modifier = (
                    round((self.player.intelligence + self.player.wisdom) / 3) - 5
                )

            if (
                modifier + dice_result < self.mob["stats"]["armor_class"]
                or dice_result == 1
            ):
                attack_successful = False
            else:
                attack_dice = self._dice(
                    attack["random_diapason"], attack["count_of_random"]
                )
                ...
                for uid, item in self.player.weapons.items():
                    damage_given += choice(
                        range(item["weapon_damage_min"], item["weapon_damage_max"])
                    )
            return {
                "attack": attack,
        ...
        def mob_attack() -> dict:
            ...
            if attack["attack_type"] == "Melee":
                modifier = (
                    round(
                        (self.mob["stats"]["strength"] + self.mob["stats"]["dexterity"])
                        / 3
                    )
                    - 5
                )
                ...
                modifier = (
                    round(
                        (self.mob["stats"]["strength"] + self.mob["stats"]["chance"])
                        / 3
                    )
                    - 5
                )
                ...
                modifier = (
                    round(
                        (
                            self.mob["stats"]["intelligence"]
                            + self.mob["stats"]["wisdom"]
                        )
                        / 3
                    )
                    - 5
            ...
            if modifier + dice_result < self.player.armor_class or dice_result == 1:
                attack_successful = False
            else:
                attack_dice = self._dice(
                    attack["random_diapason"], attack["count_of_random"]
        ...
        attacks = {"player": player_attack, "mob": mob_attack}
