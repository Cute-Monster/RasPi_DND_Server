...
class UserModule(Player):
    ...
    _base_exp_for_level: int = 300
    _exp_multiplier: int = 2.35
    ...
    def __init__(self, player_data: dict):
        ...
        self.food: dict = UtilityModule.serialize_data(player_data, "food")
        self.armor: dict = UtilityModule.serialize_data(player_data, "armor")
        self.weapons: dict = UtilityModule.serialize_data(player_data, "weapons")
        self.animals: dict = UtilityModule.serialize_data(player_data, "animals")
        self.vulnerabilities: dict = UtilityModule.serialize_data(
            player_data, "vulnerabilities"
        )
        self.attacks: dict = UtilityModule.serialize_data(player_data, "attacks")

        self.exp_for_next_level: int = self._calculate_exp_for_next_level()

    def _calculate_exp_for_next_level(self) -> int:
        return self._base_exp_for_level * (self.level ** self._exp_multiplier)

    def level_up(self, received_experience: int) -> dict:
        """
        Method for level up
